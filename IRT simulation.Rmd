...
Design <- createDesign(n_anchor = c(3, 5, 10, 15, 40), # setting the anchor to the same number as test items should mean everyone completes everything 
                       n_test_questions = c(40),
                       assumption = c("Equivalent groups", "NEAT"),
                       n_test_versions = c(2, 4),
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
    ...
    if(assumption != "NEAT") {
      mod <- manageWarnings(mirt(data, 1, itemtype=type, SE=T), warning2error = "Could not invert information matrix; model may not be (empirically) identified.")
    } else {
