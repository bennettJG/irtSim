...
```{r}
# NO anchor set, but still (a good deal of the time, though not always -- had 11 errors before reaching 100 runs)
Design0 <- createDesign(n_anchor = c(0),
                       n_test_questions = c(30),
                       anchor_type = c("random"),
                       assumption = c("Equivalent groups"),
                       n_test_versions = c(4),
                       type = c('2PL'), 
                       groupdiff = c(0, 0.4), 
                       plausible_draws = c(10),
                       n_students_total = 1000)
res0 <- runSimulation(Design0, replications = 100, parallel = T, packages = 'mirt',
                     generate=Generate, analyse=Analyse, summarise=Summarise, save_results=T)
```
