...
Design <- createDesign(n_anchor = c(3, 5, 10, 15, 20, 40), # setting the anchor to the same number as test items should mean everyone completes everything 
...
Generate <- function(condition, fixed_objects = NULL) {
    ...
    ret <- list(dat=rbind(dat1, dat2), covdata=covdata, parameters=list(Thetas=rbind(Theta1, Theta2), a=a_values, d=intercepts, groupdiff=groupdiff))
    ret
}
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
    ...
    mod <- mirt(data, 1, itemtype=type, covdata=covdata, SE=T)
    ...
    model_params <- lapply(1:length(parameters$d), function(i) c(mod@ParObjects$pars[[i]]@par, mod@ParObjects$pars[[i]]@SEpar))
    # Don't need u estimates since not testing 4PL model
    model_params_df <- data.frame(matrix(unlist(model_params), byrow=T, nrow=length(model_params)))[,c(1:3, 5:7)]
    colnames(model_params_df) <- c("a_est", "d_est", "g_est", "a_SE", "d_SE", "g_SE")
    diff_a = model_params_df[,1] - parameters$a
    diff_d = model_params_df[,2] - parameters$d
    diff_g = ifelse(type=="2PL", rep(0, length(parameters$d)), model_params_df[,3] - parameters$g)
...
Summarise <- function(condition, results, fixed_objects = NULL) {
  ...
  bias_d <- bias(do.call(c, lapply(index, function(ind, res) res[[ind]]$diff_d,
                   res=results)))
  RMSE_d <- RMSE(do.call(c, lapply(index, function(ind, res) res[[ind]]$diff_d,
                   res=results)))
  SE_d <- mean(do.call(c, lapply(index, function(ind, res) res[[ind]]$model_params_df$d_SE,
                    res=results)))
  ...
  ret <- c(bias_Theta=bias_Theta, RMSE_Theta = RMSE_Theta, mean_groupdiff=mean_groupdiff, SE_groupdiff=SE_groupdiff, bias_groupdiff=bias_groupdiff,
           ...
           bias_a = bias_a, RMSE_a = RMSE_a, SE_a = SE_a, bias_d = bias_d, RMSE_d = RMSE_d, SE_d = SE_d,
           bias_g = bias_g, RMSE_g = RMSE_g, SE_g = SE_g)
  ret
}

res <- runSimulation(Design, replications = 2, parallel = F, packages = 'mirt',
                     generate=Generate, analyse=Analyse, summarise=Summarise, save_results=F)
```
