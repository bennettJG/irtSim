...
Design <- createDesign(n_anchor = c(3, 5, 10, 15, 20, 40), # setting the anchor to the same number as test items should mean everyone completes everything 
                       ...
                       groupdiff = c(0, 0.5, 1), # Thought it would be good to see how well we could assess the size of a mean difference
...
Generate <- function(condition, fixed_objects = NULL) {
    ...
    dat <- rbind(dat1, dat2) |> make_missing(n_anchor, n_test_questions, n_test_versions)
    
    covdata <- data.frame(group = c(rep("Group 1", n_students_total/2), rep("Group 2", n_students_total/2)))
    
    ret <- list(dat=dat, covdata=covdata, parameters=list(Thetas=rbind(Theta1, Theta2), a=params$a, b=params$b, intercept=params$d, g=params$g, groupdiff=groupdiff))
    ret
}
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
    ...
    diff_Theta <- pred_theta_point[,1] - parameters$Thetas
    ...
    ret <- list(true_parameters=parameters, par_coef = pooled[2,"par"], par_SE = pooled[2, "SEpar"],
                diff_Theta = diff_Theta, pred_theta=pred_theta, se_Theta = se_Theta,
...
res <- runSimulation(Design, replications = 200, parallel = T, packages = 'mirt',
