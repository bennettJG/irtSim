...
filenames <- list.files("SimDesign-results_DESKTOP-159RJCF", pattern="*.rds", full.names=TRUE)
...
conditions <- do.call(rbind, lapply(list_results, function(i) data.frame(i$condition))) |>
  select(n_anchor, assumption, n_test_versions, groupdiff)
...
for (i in 1:length(list_results)) {
  theta_corrs[i,] <- do.call(c, lapply(1:200, function(j, res) cor(c(res[[j]]$pred_theta[,1]), y=c(res[[j]]$true_parameters$Thetas)),
                   res=list_results[[i]]$results))
}
