...
plot_bias_rmse <- function(long_data, var_name, res_var_name) {
  ...
  geom_text(data=overall_means, show.legend=F, aes(label=round(RMSE,3), y = 1.05*quantile(long_data |> filter(statistic=="RMSE") |> pull("value"), 0.95)), size=8, size.unit="pt", color="black") +
  coord_cartesian(ylim=c(.95*quantile(long_data |> filter(statistic=="RMSE") |> pull("value"), 0.01), 1.05*quantile(long_data |>  filter(statistic=="RMSE") |> pull("value"), 0.99))) +
  theme_classic() +
  paramrecovery_theme
